...
//    void updateLeft(int rightVal);
    ...
    @Query(value = "WITH RECURSIVE my_tree AS (\n" +
            ...
            ")\n" +
            "select cid,cname,ccrtdt,csize,cisdir,cparent,(case when cfullpath like '/%' then cfullpath else '/'||cfullpath end) cfullpath from my_tree where cfullpath = ?1 limit 1", nativeQuery = true)
    @QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value ="false") })
    ExtendedFileEntry findFileEntryByPath(String path);

    @Query(value = "WITH RECURSIVE my_tree AS (\n" +
            "select cid, cast('..' as text) cname,ccrtdt,csize,cisdir,cparent, coalesce(cname,'/') cfullpath, -1 corder from tfiles where cparent is null\n" +
            "union\n" +
            ...
            ")\n" +
            "select cid,cname,ccrtdt,csize,cisdir,cparent,(case when cfullpath like '/%' then cfullpath else '/'||cfullpath end) cfullpath from my_tree where cparent in (select cid from my_tree where cfullpath = ?1) order by corder",
            nativeQuery = true)
    @QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value ="false") })
    ...
    @Query(value = "WITH RECURSIVE my_tree AS (\n" +
            ...
            "select cast(0-cid as int) cid, cast('..' as text) cname,ccrtdt,csize,cisdir,cparent, coalesce(cfullpath,'/') cfullpath from my_tree where cfullpath=?1\n"
            +" union all \n"+
            "select cid,cname,ccrtdt,csize,cisdir,cparent,(case when cfullpath like '/%' then cfullpath else '/'||cfullpath end) cfullpath from my_tree where cparent in (select cid from my_tree where cfullpath = ?1)"
            , nativeQuery = true)
    @QueryHints({ @QueryHint(name = "org.hibernate.cacheable", value ="false") })
